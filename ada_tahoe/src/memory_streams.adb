...
package body Memory_Streams is
   ...
   function Contents
     (Stream : Stream_Type) return Ada.Streams.Stream_Element_Array
   is
   ...
   procedure Read
     (Stream : in out Stream_Type; Item : out Ada.Streams.Stream_Element_Array;
   ...
   begin
      if Required < Available then
         Item             :=
           Stream.Buffer (Stream.Next_Read .. Stream.Next_Read + Required - 1);
   ...
   procedure Set_Contents
     (From : Ada.Streams.Stream_Element_Array; Stream : in out Stream_Type)
   is
      use type Ada.Streams.Stream_Element_Offset;
   begin
      Reset (Stream);
   ...
   procedure Write
     (Stream : in out Stream_Type; Item : in Ada.Streams.Stream_Element_Array)
   is
      use type Ada.Streams.Stream_Element_Offset;
   begin
      ...
      Stream.Buffer
        (Stream.Next_Write .. Stream.Next_Write + Item'Length - 1) :=
        Item;
      Stream.Next_Write := Stream.Next_Write + Item'Length;
   end Write;

   procedure Write_Contents
     (To : access Ada.Streams.Root_Stream_Type'Class; Stream : Stream_Type)
   is
      use type Ada.Streams.Stream_Element_Offset;
   begin
      Ada.Streams.Write
