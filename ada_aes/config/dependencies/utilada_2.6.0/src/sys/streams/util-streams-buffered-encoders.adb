...
package body Util.Streams.Buffered.Encoders is
   ...
   overriding procedure Initialize
     (Stream : in out Encoder_Stream; Size : in Positive)
   is
   begin
   ...
   --   overriding
   procedure Produces
     (Stream : in out Encoder_Stream; Output : access Output_Stream'Class;
      Size   : in     Positive)
   is
   begin
   ...
   --  -----------------------
   procedure Consumes
     (Stream : in out Encoder_Stream; Input : access Input_Stream'Class;
      Size   : in     Positive)
   is
   begin
      Stream.Initialize (Size);
   ...
   --  Close the sink.
   --  -----------------------
   overriding procedure Close (Stream : in out Encoder_Stream) is
   begin
      Stream.Flush;
      ...
      while First_Encoded <= Buffer'Last loop
         ...
         Stream.Transform.Transform
           (Data => Buffer (First_Encoded .. Buffer'Last),
            ...
            Last => Last_Pos, Encoded => Last_Encoded);
         if Last_Encoded < Buffer'Last or else Last_Pos = Stream.Buffer'Last
         then
            Stream.Output.Write
              (Stream.Buffer (Stream.Buffer'First .. Last_Pos));
   ...
   begin
      if Stream.Input = null then
         ...
         Stream.Input.Read
           (Stream.Buffer (1 .. Stream.Last), Stream.Write_Pos);
         ...
         if Avail = 0 then
            ...
            if Avail <= 0 then
               ...
               Stream.Transform.Finish
                 (Into => Into (Start .. Into'Last), Last => Last);
               return;
            end if;
         end if;
         Stream.Transform.Transform
           (Data    => Stream.Buffer (Pos .. Pos + Avail - 1),
            Into    => Into (Start .. Into'Last), Last => Last_Pos,
   ...
   --  -----------------------
   overriding procedure Flush (Stream : in out Encoder_Stream) is
      Last_Pos : Ada.Streams.Stream_Element_Offset := Stream.Write_Pos - 1;
   begin
      if not Stream.Flushed and then Stream.Buffer /= null
        and then Stream.Write_Pos >= Stream.Buffer'First
      then
         Stream.Transform.Finish
           (Stream.Buffer (Stream.Write_Pos .. Stream.Buffer'Last), Last_Pos);
         Stream.Write_Pos := Last_Pos + 1;
         if not Stream.No_Flush then
            if Stream.Write_Pos > 1 then
               if Stream.Output /= null then
                  Stream.Output.Write
                    (Stream.Buffer (1 .. Stream.Write_Pos - 1));
               end if;
               Stream.Write_Pos := 1;
            end if;
            if Stream.Output /= null then
   ...
   overriding procedure Finalize (Stream : in out Encoder_Stream) is
   begin
